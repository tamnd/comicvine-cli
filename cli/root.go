// Package cli assembles the comicvine command tree from the comicvine
...
// NewApp assembles the kit application from the comicvine domain. The
...
//
// To add a command, declare it in comicvine/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := comicvine.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(comicvine.Domain{}).Register(app)
