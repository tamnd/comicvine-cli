...
describes what users get. Pick a format with `-o`, or let comicvine choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
comicvine <command> -o table   # aligned columns for reading
comicvine <command> -o jsonl   # one JSON object per line, for piping
comicvine <command> -o json    # a single JSON array
comicvine <command> -o csv     # spreadsheet friendly
comicvine <command> -o tsv     # tab-separated
comicvine <command> -o url     # just the URL column
comicvine <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
comicvine <command> --fields id,title,url
```
...
```bash
comicvine <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
comicvine <command>            # a table, because this is a terminal
comicvine <command> | wc -l    # JSONL, because this is a pipe
```
